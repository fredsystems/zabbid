...
use crate::queries::canonical::{lookup_area_id, lookup_bid_year_id};

/// Inserts a new user from the last user in the state.
///
...
pub fn insert_new_user(conn: &mut SqliteConnection, state: &State) -> Result<(), PersistenceError> {
    let user = state
        .users
        .last()
    ...
    // Look up the IDs
    let bid_year_id: i64 = lookup_bid_year_id(conn, user.bid_year.year())?;
    let area_id: i64 = lookup_area_id(conn, bid_year_id, user.area.id())?;
...
pub fn sync_canonical_users(
    conn: &mut SqliteConnection,
    state: &State,
) -> Result<(), PersistenceError> {
    // Look up the IDs
    let bid_year_id: i64 = lookup_bid_year_id(conn, state.bid_year.year())?;
    let area_id: i64 = lookup_area_id(conn, bid_year_id, state.area.id())?;
